...
int MainWindow::serial_read()
{
    ...
    m_RecData = RecData_HEX.toHex().toUpper();                      // toHex(" ").toUpper();
    m_RecData = Add_Space(0,m_RecData);                             // 为了适配5.5.1的aarch编译器
...
void MainWindow::on_ID_write_btn_clicked()
{
    if(m_Com_Open){
        ...
        else
        {
            ...
            Send_Data_string = Add_Space(0,Send_Data_string);
            m_ID_flag|=BIT_1;
